...
# holdout train/test + scaler : utilisés par la validation approfondie du
# meilleur modèle (rapport de classification, courbe d'apprentissage) en fin de
# script. la comparaison des 10 modèles, elle, utilise la validation croisée.
...
print(classification_report(y_test, y_pred_rf, labels=SEV_ORDER,
                            target_names=SEV_ORDER, zero_division=0))

cv5 = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)   # folds mélangés (cohérent avec le haut)
cv_scores = cross_val_score(rf_best, scaler.transform(X), y, cv=cv5, scoring="f1_macro", n_jobs=-1)
...
train_sizes, train_scores, val_scores = learning_curve(
    RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1,
                           class_weight="balanced"),
    scaler.transform(X), y, cv=cv5, scoring="f1_macro",
